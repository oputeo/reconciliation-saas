-- ============================================================
-- ReconFlow Phase B: Rule Change Approval Workflow
-- ============================================================
...
--
-- Requires: RUN_RECONCILIATION_RULES.sql (Phase A) applied first
--
-- Creates:
--   • reconciliation_rule_changes (pending/approved/rejected/cancelled)
--   • user_has_min_role(uuid, text)
--   • approve_reconciliation_rule_change(uuid, text)
--   • reject_reconciliation_rule_change(uuid, text)
--
-- Tightens catalog RLS: direct edits → admin only (auditors propose changes)
-- ============================================================
...
-- Guard: Phase A catalog must exist
DO $$
BEGIN
  IF to_regclass('public.reconciliation_rule_catalog') IS NULL THEN
    RAISE EXCEPTION 'Run RUN_RECONCILIATION_RULES.sql (Phase A) before this script.';
  END IF;
END;
$$;
...
-- ---------- Approve pending change → publish to catalog ----------
...
-- ---------- Tighten catalog: admin-only direct edits ----------
...
-- ---------- Audit log insert (propose/cancel from client) ----------
...
SELECT
  to_regclass('public.reconciliation_rule_changes') AS changes_table,
  to_regprocedure('public.approve_reconciliation_rule_change(uuid,text)') AS approve_rpc,
  to_regprocedure('public.reject_reconciliation_rule_change(uuid,text)') AS reject_rpc,
  to_regprocedure('public.user_has_min_role(uuid,text)') AS min_role_fn;

SELECT status, COUNT(*) AS change_count
FROM public.reconciliation_rule_changes
